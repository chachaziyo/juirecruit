<template>
  <div id="comnCodMgr">
    ...
    <div class="divComGrpCodList">
      <table class="col">
        ...
        <tbody>
          ...
          <template v-else>
            <tr v-for="item in items" :key="item.grp_cod"><!-- npm방식에는 for문에 :key꼭들어가야함! -->
              <td>{{ item.indexNew }}</td>
    ...
    <!--  v-model="currentPage" : 페이지번호를 눌렀을때 누른 번호를 변수에 담기위하여 v-model 담아옴
      :click-handler="clickCallback" : 페이지를 눌렀을때 실행할 이름!
    -->
...
import Paginate from 'vuejs-paginate-next';//npm인스톨에서 설치 미리 해놓은것!

export default {//export default 로 시작됨! , el옵션이 없음!
  data: function () {
    return {
      items: [],
      groupDetail: [],
  ...
  mounted() {////실제 만들어진것을 초기화면으로 만들어질때(=onload 랑 같음 = $(document).ready(function)
    this.listsearch();//화면열자마자 이것부터 실행해! 라는 뜻
  },
  methods: {
    // uwfa
    searchdetail: function (group_code) {
      ...
      this.axios
        .post('/system/vuelistComnDtlCod.do', params)
    ...
    rowClicked: async function (grpcd) {
      if (grpcd == null || grpcd == '') {
        ...
        const modal = await openModal(grddetailModal, {//openModal모달열어라
          title: '그룹코드 등록',
          grpcd: '',
          action: this.action,
        });
        ...
        modal.onclose = (popupparam) => {//팝업창이 닫히면 listsearch여기로 돌아간다 ,popupparam아무 의미x 지워야하는데 안지웠다함
          console.log('Close : ' + popupparam);
          this.listsearch();
          //return false; //Modal will not be closed
        };
    ...
    listsearch: function () {
      ...
      let params = new URLSearchParams();
      params.append('currentPage', this.currentPage);//currentPage 초기값이 1
